...
calibracion_jerarquica <- function(df_segmentos, total_nacional) {
  ...
  # Se colapsa primero a una fila por municipio (igual que el Paso 1 colapsó
  # a una fila por departamento); sumar N_mun_hat sin colapsar repetiría el
  # total de cada municipio una vez por segmento y produciría un phi_mun
  # incorrecto.
  municipios <- df_segmentos %>%
    group_by(departamento, municipio) %>%
    summarise(N_mun_hat = sum(pob_media), .groups = "drop") %>%
    left_join(dptos %>% select(departamento, N_dpto_bench), by = "departamento") %>%
    group_by(departamento) %>%
    mutate(
      phi_mun     = N_dpto_bench / sum(N_mun_hat),
      N_mun_bench = N_mun_hat * phi_mun
    ) %>%
    ungroup()

  # Paso 3: calibrar segmentos contra el total municipal ya calibrado
  df_segmentos %>%
    left_join(
      municipios %>% select(departamento, municipio, N_mun_hat, N_mun_bench),
      by = c("departamento", "municipio")
    ) %>%
    group_by(departamento, municipio) %>%
    mutate(
      phi_seg   = N_mun_bench / N_mun_hat,
      pob_bench = pob_media * phi_seg
    ) %>%
    ungroup()
}
```
