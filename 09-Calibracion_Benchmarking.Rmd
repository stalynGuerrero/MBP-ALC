# Calibración y Benchmarking Subnacional
...
$$
\kappa_k = \frac{\hat{N}_k^{bench} - \hat{N}_k}{\hat{N}_k}
$$

Bajo el ajuste proporcional, $\kappa_k = \phi - 1$ es idéntico para todas las unidades, independientemente de su tamaño o de la precisión de su estimación. Bajo el ajuste ponderado por precisión, $\kappa_k$ varía sistemáticamente entre unidades: se espera una relación positiva entre $\kappa_k$ y la incertidumbre posterior relativa de la unidad, de modo que las unidades con estimaciones más precisas —mayor cobertura censal directa— exhiban valores de $\kappa_k$ cercanos a cero, mientras que las unidades con mayor dependencia de la estructura jerárquica del modelo absorban una proporción mayor del ajuste:

```r
cambio_relativo <- estimaciones_departamento %>%
  mutate(
    kappa_prop = (N_bench_prop - tot_pob_media) / tot_pob_media,
    kappa_pond = (N_bench_pond - tot_pob_media) / tot_pob_media
  )

# Relación entre el cambio relativo y la incertidumbre posterior
ggplot(cambio_relativo, aes(x = tot_pob_sd / tot_pob_media, y = kappa_pond)) +
  geom_point() +
  labs(x = "Coeficiente de variación posterior",
       y = "Cambio relativo tras calibración ponderada")
```

Un patrón en el que las unidades con mayor coeficiente de variación posterior muestran mayores valores de $|\kappa_k|$ confirma que el ajuste ponderado está operando de acuerdo con el principio que lo motiva: concentrar la corrección donde la información propia del modelo es más débil. La ausencia de este patrón —o su presencia inversa, con grandes correcciones sobre unidades de alta precisión— señala que los pesos utilizados no reflejan adecuadamente la incertidumbre relativa de las unidades y debe revisarse su especificación.
