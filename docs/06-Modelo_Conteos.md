...
**Nota de notación.** La cardinalidad $D = |\mathcal{S}|$ corresponde al mismo conjunto de segmentos denotado $\{1, \ldots, I\}$ en los Capítulos \@ref(cap-modelos-poblacion) y \@ref(cap-insumos); se adopta aquí la notación $D$ ($D_{obs}$, $D_{tot}$) por su correspondencia directa con la implementación computacional en Stan (Sección \@ref(stan-implementacion) del capítulo siguiente). El número de estructuras habitacionales $V_i$ es el mismo preconteo introducido en el Capítulo \@ref(cap-integracion). Finalmente, ni $D$ ni sus variantes indexadas deben confundirse con la densidad poblacional por área $D_i = N_i/A_i$ definida en el Capítulo \@ref(cap-modelos-poblacion): la densidad de personas por estructura habitacional propia de este capítulo se denota $\delta_i$.
